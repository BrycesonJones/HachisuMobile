...
Deno.serve(async (req) => {
  ...
  // Product gate, enforced server-side: Lightning settings changes must not be
  ...
  if (!LIGHTNING_ENABLED) return lightningDisabledResponse('update-lightning-settings');
  ...
  if (!store || store.user_id !== user.id) {
    logAuthorizationDenied({
      action: 'update-lightning-settings',
      userId: user.id,
